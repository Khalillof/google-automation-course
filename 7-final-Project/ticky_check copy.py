...
def logs_search(log_file, err_names_pattern):
    ...
    with open(log_file, mode='r', encoding='UTF-8') as file:
        for log_line in file.readlines():
            error_patterns = []
            for i in range(len(err_names_pattern.split(' '))):
                error_patterns.append(r"{}".format(
                    err_names_pattern.split(' ')[i]))
            if all(re.search(error_pattern, log_line) for error_pattern in error_patterns):
    ...
    try:
        with open(report_url, 'w+', newline="") as output_file:
            ...
            for item in group_lists:
                temDic.update({item[0]:len(item)})
            for k,v in sorted(temDic.items(), key = operator.itemgetter(1), reverse=True):
...
def remove_text_in_between(left, right, _text):
    # remove txt in between two chars
...
def clean_text_recusive(_text):
    if '[' and ']' in _text:
        return clean_text_recusive(remove_text_in_between('[', ']', _text))
    elif '(' and ')' in _text:
        return clean_text_recusive(remove_text_in_between('(', ')', _text))
    else:
        return _text
...
def process_str(str_list):
    grp_list = []
    for line in str_list:
        sub_line = line[41:]
        _clean = clean_text_recusive(sub_line).strip()
        grp_list.append(_clean)
    return [list(items) for gr, items in groupby(sorted(grp_list))]
...
def main():
    ...
    error_toCsv("error_message.csv", process_str(sort_by_frequency(
        [x.strip() for x in two_lists])), ['Error', 'Count'])

    nestedDics_toCsv("user_statistics.csv", users_entries,
                     ['Username', info, _err])
